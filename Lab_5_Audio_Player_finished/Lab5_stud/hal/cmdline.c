...
//*****************************************************************************
int
CmdLineProcess(char *pcCmdLine)  // argument = string from the console
{
    static char *argv[CMDLINE_MAX_ARGS + 1];
	
	int argc;					// argument counter (for decoding string into argument-pieces)

	
    char *pcChar;				// String for receiving the argument string

    tCmdLineEntry *pCmdEntry;
   int bFindArg = 1;			// temporary flag to mention, that a new argument is comming
	argc = 0;					// Initialize the argument counter
 
	pcChar = pcCmdLine;			// Argument string temporary stored.
								// STRING = Pointet to the first char.
								// By increasing the pointer-adress, the next char is readed out
    ...
    {
        // **********************************************************************
        // If the value is a space, then replace it with a zero, and set the flag
        // to search for the next argument.
        // ***********************************************************************
        if(*pcChar == ' ')
        {
            *pcChar = 0;  	// replace the value with a 0.
            bFindArg = 1;   // temporary flag, to mention, a new argument will come
        }
        ...
        else
        {
            ...
            if(bFindArg)
            {
                ...
                //
                if(argc < CMDLINE_MAX_ARGS) // error checking
                {
                    argv[argc] = pcChar;
                    argc++;
                    bFindArg = 0;
    ...
    {
        //
        // Start at the beginning (first adress) of the command table, 
		// to look for a matching command.
        //
        pCmdEntry = &g_sCmdTable[0];  // Adressübergabe
        ...
        while(pCmdEntry->pcCmd)
        {
            ...
            // arguments.
            // *******************************************************************
			// Kommt das Kommando (pCmdEntry->pcCmd) in der Tabelle vor (argv[0])
			// Dann soll die Funktion aufgerufen werden (pCmdEntry->pfnCmd)
			// *******************************************************************
